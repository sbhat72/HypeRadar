...
- **pom.xml**: Removed hardcoded `<profiles><profile>local</profile></profiles>` from the spring-boot-maven-plugin configuration. Profile selection now comes from `SPRING_PROFILES_ACTIVE` or `-Dspring-boot.run.profiles` — the plugin entry itself is intact.
- **TickerController**: Added 5 s connect / 8 s read timeouts on `yahooRestTemplate` via `SimpleClientHttpRequestFactory`. Added `MAX_LIMIT = 50` cap on the `limit` param. Added ticker symbol validation (regex `[A-Z]{1,5}`) before building the Yahoo URL. Added null + zero guard on `chartPreviousClose` to prevent `Infinity`/`NaN` in the response. Added `slf4j` `log.warn(...)` in the catch block so failures are diagnosable. Removed unused imports.
- **RedisBootUpService**: Added missing `@Service` stereotype annotation so Spring picks up the bean and `@PostConstruct` actually runs on startup. Renamed the `forEach` lambda parameter from `ticker` to `hypeScore` to match the iterated type.
- **hyped-stocks/page.tsx**: `TrendingTickerDto` interface now includes `price`, `priceChange`, `changePercent` from the backend. `fetchTickers` maps these fields directly instead of firing a separate `fetchYahooPrice` / `/api/yahoo-finance` call per symbol, eliminating up to 20 redundant client-side Yahoo fetches on each page load.
- **Docs**: Updated `context/CLAUDE.md`, `context/project-overview.md`, `context/feature-specs/02-hyped-stocks.md`, `03-ticker-deep-dive.md`, `11-frontend-wiring.md`, `14-FE-Problems.md`, and `context/scaffold-prompt.md` to reflect current stack (Yahoo Finance instead of Finnhub/Alpha Vantage/Reuters, StockTwits instead of Reddit, correct routes and component paths, updated `useApiClient` example with null-token guard and 204 handling, corrected `TrendingTickerDto` field mapping).
...
| 20 | Alpha Vantage GLOBAL_QUOTE free tier capped at 25 req/day — 100+ tickers returned null price for most trending cards | Resolved | Two-layer fix: (1) `TickerController` replaced Alpha Vantage with Yahoo Finance v8 chart API via RestTemplate (silent fail per ticker). (2) Java server-side calls are blocked by Yahoo Finance's TLS fingerprinting regardless of User-Agent — prices are now fetched client-side in `hyped-stocks/page.tsx` via the existing `/api/yahoo-finance` Next.js proxy (Node fetch passes Yahoo's checks). `fetchYahooPrice` runs in parallel via `Promise.all` for all 20 tickers. Loading skeleton stays up until both trending data and prices are ready. `TrendingTickerCard` change display refactored: `hasChange`/`isPositive`/`changeColor` derived from null-aware checks; null renders as `--` in `text-secondary` with no sign prefix. |
| 21 | `RedisBootUpService` missing `@Service` annotation — `@PostConstruct` warm-up never ran at startup | Resolved | Added `@Service` to `RedisBootUpService`. Also renamed the `forEach` lambda param from `ticker` to `hypeScore` to match the `HypeScore` entity being iterated. |
| 22 | `pom.xml` hardcoded `<profiles><profile>local</profile></profiles>` in spring-boot-maven-plugin, forcing the `local` Spring profile on every `spring-boot:run` invocation and overriding `application-default.properties` | Resolved | Removed the `<configuration>` block from the plugin entry. Profile selection is now controlled via `SPRING_PROFILES_ACTIVE` env var or `-Dspring-boot.run.profiles` flag at invocation time. |
| 23 | `TickerController.yahooRestTemplate` had no connect/read timeouts — slow Yahoo responses could hang a trending request indefinitely | Resolved | Replaced `new RestTemplate()` with a `RestTemplate` backed by `SimpleClientHttpRequestFactory` (5 s connect, 8 s read). Also added `MAX_LIMIT = 50` cap, ticker symbol regex validation, null+zero guard on `chartPreviousClose`, and `log.warn` in the exception catch path. |
| 24 | `hyped-stocks/page.tsx` fetched Yahoo Finance separately for every ticker after receiving the trending list, doubling the work already done by `TickerController` | Resolved | Extended `TrendingTickerDto` interface in the frontend to include `price`, `priceChange`, `changePercent`. `fetchTickers` now maps these backend-provided fields directly, removing the `fetchYahooPrice` helper and the `Promise.all` of up to 20 extra client-side fetches. |
| 25 | `RssFeedParserService.loadValidTickers()` called Alpha Vantage LISTING_STATUS with no HTTP timeout — when the free tier 25 req/day limit is hit the request hangs forever and the backend never finishes starting | Resolved | (1) `RssFeedParserService` constructor now builds `RestTemplate` via `SimpleClientHttpRequestFactory` (10 s connect, 10 s read). `loadValidTickers()` wrapped in try-catch: on any exception logs a warning and leaves `validTickers` empty. (2) `findTickerMentions()` changed from `if (validTickers.contains(symbol))` to `if (!validTickers.isEmpty() && !validTickers.contains(symbol)) continue` — when the ticker universe is empty all discovered symbols are accepted so the pipeline runs normally. (3) `loadSentimentDictionary()` signature changed from `throws IOException` to a try-catch that logs a warning and returns on any I/O failure. Backend now always starts within 10 s regardless of Alpha Vantage reachability. |
