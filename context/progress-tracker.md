...
- `Navbar` (`src/components/ui/Navbar.tsx`) — sticky top nav for all protected pages. Left: HypeRadar logo linking to `/hyped-stocks`. Centre: Dashboard, Watchlist, and Alerts links with green underline on active route. Right: Clerk `<UserButton />`
